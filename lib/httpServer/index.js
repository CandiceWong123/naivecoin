...
// Page access routing, displaying attendance records
        this.app.get('/attendance', (req, res) => {
            if (req.headers['accept'] && req.headers['accept'].includes('text/html')) {
                // Display attendance records when accessing the attendance page
                res.render('attendance/index.pug', {
                    pageTitle: 'Attendance Records',
                    records: blockchain.getAttendanceRecords() 
                    // Assuming that the blockchain object has a getAttendanceRecords method
                });
            } else {
        ...
        // API routing to obtain attendance record list
        this.app.get('/blockchain/attendance', (req, res) => {
            res.status(200).send(blockchain.getAttendanceRecords());
        });

        // API routing to retrieve individual attendance records
        this.app.get('/blockchain/attendance/:transactionId([a-zA-Z0-9]{64})', (req, res) => {
            let attendanceRecord = blockchain.getAttendanceRecordByTransactionId(req.params.transactionId);
            if (attendanceRecord == null) throw new HTTPError(404, `Attendance record not found for transaction ID '${req.params.transactionId}'`);
            res.status(200).send(attendanceRecord);
        });

        // Add attendance records (through transactions)
...
console.log(requestTransaction)
        ...
        this.app.post('/blockchain/fork', (req, res) => {
            const competingChain = req.body.blocks;  // Retrieve competitive chains from requests
        
            if (!competingChain || !Array.isArray(competingChain)) {
                return res.status(400).send('Invalid chain data');
            }
        
            blockchain.checkForForkAndResolve(competingChain);  // Check and analyze forks
        
            res.status(200).send({ message: 'Fork check complete' });
        });
        ...
        this.app.post('/blockchain/addAttendanceTransaction', (req, res) => {
