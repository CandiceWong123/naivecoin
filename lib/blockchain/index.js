...
class Blockchain {
    constructor(dbName) {
        ...
        this.difficulty = 4; // Initial difficulty value
        this.adjustmentInterval = 10; // Adjust every 10 blocks
        this.targetTime = 10 * 60; // Each block has a target time of 10 minutes (in seconds)
    
        this.emitter = new EventEmitter();
        this.init();
    }
    adjustDifficulty() {
        const newDifficulty = this.getDifficulty();
    if (newDifficulty !== this.difficulty) {
        console.info(`Difficulty adjusted from ${this.difficulty} to ${newDifficulty}`);
        this.difficulty = newDifficulty;
    }
        const length = this.blocks.length;
    
        // Check if there are enough blocks available for computation
        if (length < this.adjustmentInterval + 1) {
            console.warn('Not enough blocks to adjust difficulty.');
            return this.difficulty;
        }
    
        // Retrieve the most recent block and adjust the starting block of the window
        const lastBlock = this.blocks[length - 1];
        const adjustmentBlock = this.blocks[length - 1 - this.adjustmentInterval];
    
        // Actual generate time
        const actualTime = lastBlock.timestamp - adjustmentBlock.timestamp;
        // Expected generate time
        const expectedTime = this.adjustmentInterval * this.targetTime;
    
        // Dynamically adjust difficulty logic
        if (actualTime < expectedTime / 2) {
            this.difficulty += 1; // increase difficulty
        } else if (actualTime > expectedTime * 2) {
            this.difficulty = Math.max(this.difficulty - 1, 1); 
            // decrease difficulty, the lowest difficulty is 1
        }
    
        console.debug(`Adjusted difficulty: ${this.difficulty}`);
        return this.difficulty;
    }
    ...
    // Retrieve attendance records based on transaction ID
    getAttendanceRecordByTransactionId(transactionId) {
        // Search for all attendance records, assuming each attendance record is associated with a transaction ID
        return this.attendanceRecords.find(record => record.transactionId === transactionId);
    }

    // Simulate adding attendance records
    addAttendanceRecord(transactionId, studentId, timestamp) {
        const newRecord = {
    ...
    // Basic Fork Analysis: Analyzing Competitive Chains
    resolveFork(competingChain) {
        // Compare the length of the current chain with the competing chain and choose a longer chain
        if (competingChain.length > this.blocks.length) {
            console.log('Detected fork. Switching to the competing chain...');
            // Update the current chain to a longer chain
            this.blocks = competingChain;
        } else {
            console.log('No fork detected. Current chain is the longest.');
        }
    }

    // Check for forks and handle them
    checkForForkAndResolve(newBlocks) {
        const currentChainLength = this.blocks.length;
        const competingChainLength = newBlocks.length;

        // If the competition chain is longer, call the fork parsing method
        if (competingChainLength > currentChainLength) {
            this.resolveFork(newBlocks);
        }
    }
    ...
    addBlock(newBlock, emit = true) {
        // Only add to the blockchain when the new block is valid
        if (this.checkBlock(newBlock, this.getLastBlock())) {
            this.blocks.push(newBlock);
            this.blocksDb.write(this.blocks);
    
            // Remove included transactions from the pending transaction list
    ...
    getAttendanceRecords() {
        // Filter out the attendance records from the transactions
        return this.transactions.filter(transaction => transaction.type === 'attendance');
    }
    ...
    // getAllRegistration(){
    //     // let blocks = this.blocksDb.read(Blocks);
    //     const map = new Map()
    //     this.blocks.forEach((block) => {
    //         if(block.transactions[0].type == "registration"){
    //             // console.log(block.transactions[0].data.outputs[0]);
    //             map.set(block.transactions[0].data.outputs[0].address,block.transactions[0].data.outputs[0].studentID);
    //         }
    //     })
    //     return map;
    // }

    // // Get registration record searching with courseId
    // getCourseAttendance(courseId){
    //     const _2dArr = [];
    //     this.blocks.forEach((block) => {
    //         if(block.transactions[0].type == "attendance"){
    //             if(block.transactions[0].data.outputs[0].course == courseId){
    //                 _2dArr.push([block.transactions[0].data.outputs[0].studentId,block.transactions[0].timestamp]);
    //             }
    //         }
    //     })
    //     console.log(_2dArr);
